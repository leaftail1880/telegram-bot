...
export function log(msg, extra = {}) {
    console.log(msg);
    data.debug
    ? bot.telegram.sendMessage(data.chatID.owner, msg, extra): bot.telegram.sendMessage(data.chatID.log, msg, extra);
}
...
async function start() {
    ...
    *========================**/
    ...
    for (const plugin of config.modules) {
        ...
        plgs.push(`${plugin} (${(performance.now() - start).toFixed(2)} ms)`);
    }
